    ...
    public function ListingReviews(Request $request, $listing_id)
{
   ...
   public function contact_store(Request $request)
   {
    ...
    public function gallery(Request $request)
    {
        ...
        $page_data['galleries'] = $query->orderBy('sort_order', 'asc')
                                       ->orderBy('created_at', 'desc')
                                       ->paginate(12);
        
        $page_data['categories'] = Gallery::getCategories();
        
        // Group galleries by category for organized display
        $page_data['galleryByCategory'] = Gallery::active()
                                                ->orderBy('sort_order', 'asc')
                                                ->get()
                                                ->groupBy('category');
    ...
    public function team(Request $request)
    {
        ...
        $page_data['teamMembers'] = $query->ordered()->get();
        
        $page_data['categories'] = TeamMember::select('category')
                                            ->distinct()
        ...
        return view('frontend.team', $page_data);
    }

    // Get active regions for frontend display (used in homepage slider)
