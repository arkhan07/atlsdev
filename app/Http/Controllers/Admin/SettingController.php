...
// Tambahkan ke app/Http/Controllers/Admin/SettingController.php

    /**
     * WhatsApp OTP Settings
     */
    public function whatsapp_otp_settings(){
        return view('admin.setting.whatsapp_otp_settings');
    }

    public function whatsapp_otp_settings_update(Request $request) {
        $data = $request->all();
        
        // OneSender API Configuration
        if(System_setting::where('key', 'onesender_api_url')->get()->count() > 0) {
            System_setting::where('key', 'onesender_api_url')->update([
                'key' => 'onesender_api_url',
                'value' => sanitize($data['onesender_api_url']),
            ]);
        } else {
            System_setting::create([
                'key' => 'onesender_api_url',
        ...
        if(System_setting::where('key', 'onesender_api_key')->get()->count() > 0) {
            System_setting::where('key', 'onesender_api_key')->update([
                'key' => 'onesender_api_key',
                'value' => sanitize($data['onesender_api_key']),
            ]);
        } else {
            System_setting::create([
                'key' => 'onesender_api_key',
        ...
        if(System_setting::where('key', 'whatsapp_otp_enabled')->get()->count() > 0) {
            System_setting::where('key', 'whatsapp_otp_enabled')->update([
                'key' => 'whatsapp_otp_enabled',
                'value' => $data['whatsapp_otp_enabled'] ?? 0,
            ]);
        } else {
            System_setting::create([
                'key' => 'whatsapp_otp_enabled',
        ...
        if(System_setting::where('key', 'otp_login_message')->get()->count() > 0) {
            System_setting::where('key', 'otp_login_message')->update([
                'key' => 'otp_login_message',
                'value' => $data['otp_login_message'],
            ]);
        } else {
            System_setting::create([
                'key' => 'otp_login_message',
        ...
        if(System_setting::where('key', 'otp_register_message')->get()->count() > 0) {
            System_setting::where('key', 'otp_register_message')->update([
                'key' => 'otp_register_message',
                'value' => $data['otp_register_message'],
            ]);
        } else {
            System_setting::create([
                'key' => 'otp_register_message',
        ...
        if(System_setting::where('key', 'otp_welcome_message')->get()->count() > 0) {
            System_setting::where('key', 'otp_welcome_message')->update([
                'key' => 'otp_welcome_message',
                'value' => $data['otp_welcome_message'],
            ]);
        } else {
            System_setting::create([
                'key' => 'otp_welcome_message',
        ...
        Session::flash('success', get_phrase('WhatsApp OTP settings updated successfully!'));
        return redirect()->back();
    }

    /**
     * Test OneSender connection
     */
    public function test_onesender_connection(Request $request) {
        $oneSenderService = new \App\Services\OneSenderService();
        $result = $oneSenderService->testConnection();
